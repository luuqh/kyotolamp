      ...
      integer*8:: id,nm,pm,n0,p0,px,n,h,k,p,q,marktp,um,shift,search
      integer*4:: ix,iy,iz,color,hournum,hourstep
      character*80::f_pathin,f_pathout,f_in,f_out
      character*8:: f_prefix,number
      character*2:: f_suffix
      character*4:: f_type,f_number

      ! basic settings
      n0 = 0      ! first step
      nm = 24*15/2  ! number of steps
      dt = 3600.*2  ! time interval [s]
      f_type = '.ga' !'.g01'
      ...
      f_in = '../../lamp/lab00/merge/00000000'//trim(f_type)
      write(*,*)f_in
      ...
      do k = 0,hournum
         ...
         if (h.lt.0) then 
            ...
            call i2s (h,2,f_suffix)         
            f_number = trim(f_suffix)
         endif
            do p = 1,p0
               trj(p) = 0
            enddo
            f_pathin = '../../lamp/lab'//trim(f_number)//'/merge/'
            f_pathout = '../../lamp/lab'//trim(f_number)//'/merge/'

            ! read last number of particles and then calculate the number of missing particles
            time = nm*dt
            call timing (time,f_prefix)
            f_in = trim(f_pathin)//trim(f_prefix)//trim(f_type)
            write(*,*)f_in
            ...
            um = nm + 1
            f_out = trim(f_pathout)//'trajmis.ea'
