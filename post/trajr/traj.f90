      ...
      integer*8:: id,nm,pm,n0,p0,px,pn,n,p,q,marktp,um,shift,search
      integer*4:: ix,iy,iz,color,k,km,k0,filext
      character*80::f_pathin,f_pathout,f_in,f_out
      character*8:: f_prefix,number,f_suffix
      character*8:: f_suffix2

      ! basic settings
      f_pathin = '../../lamp/lab4/data/' 
      f_pathout = '../../lamp/lab4/merge/'
      n0 = 0      ! first step
      nm = 150-1 !(27*24+6)/3 !/24  ! number of steps
      dt = 3600*24 !*3 !*24 ! time interval [s]
      k0 = 1
      km = 39
      filext = 2 !file extension .p**** or .p**

      pn = 720 !60 !2624 !int(nm)+1
      allocate (ip(pn))
      allocate (trj(pn))
      ...
      do k = k0,km
         ...
         call i2s (k,filext,f_suffix2)
         f_suffix = trim(f_suffix2)
         time = n0*dt
         ...
         if (p0.gt.pn) then
            write(*,*)'Allocation error. Plz increase [pn] to ',p0
         else
